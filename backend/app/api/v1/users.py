...
from app.core.constants import ActivityEventType, Permission
...
async def list_roles(
    ...
    """Real per-org roles + granted permissions — replaces the frontend's old
    static, read-only permissionMatrix mock (BACKEND_READINESS_REPORT.md §6)."""
    result = await db.execute(select(Role).where(Role.organization_id == tenant.organization_id).order_by(Role.name))
    ...
    for role in roles:
        ...
        out.append(
            RoleOut(id=role.id, name=role.name, is_system=role.is_system, permissions=list(perms_result.scalars().all()))
        )
    return out
...
async def remove_team_member(
    ...
    await log_activity(
        db,
        actor_user_id=tenant.membership.user_id,
        event_type=ActivityEventType.TEAM_MEMBER_REMOVED,
        summary=f"Removed team member {removed_user_id}",
        organization_id=tenant.organization_id,
        metadata={"removed_user_id": str(removed_user_id)},
    )
    await db.commit()
