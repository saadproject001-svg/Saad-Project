  ...
  return (
    <header className="h-16 bg-neu-bg flex items-center px-4 md:px-8 gap-3 md:gap-7">
      ...
      <div className="ml-auto flex items-center gap-3 md:gap-7 text-neu-muted">
        ...
        {user?.avatar_url ? (
          <img className="w-9 h-9 rounded-full cursor-pointer shrink-0 neu-soft p-0.5" src={user.avatar_url} alt="Profile" />
        ) : (
          <div className="w-9 h-9 rounded-full cursor-pointer shrink-0 neu-soft flex items-center justify-center text-xs font-semibold text-neu-text">
            {initialsOf(user)}
          </div>
        )}
      </div>
